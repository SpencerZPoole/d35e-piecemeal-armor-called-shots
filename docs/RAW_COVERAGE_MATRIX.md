...
| PF1e piecemeal armor | RAW-adapted default | Uses the supplied Ultimate Combat rules for piece categories, complete suits, worst ACP/ASF/max Dex, mixed-suit ASF, hasty donning, all-active-piece material checks, and magic/masterwork precedence where D35E can represent the result. |
...
| PAcS armor piece items | D35E-calibrated convenience pack | The `PAcS Armor Pieces` compendium ships ready-to-use torso, arm, and leg override items generated from the calibrated catalog. PAcS slots accept those flagged pieces directly; vanilla full suits can be broken down first. Torso-only armor and other non-PAcS items are rejected instead. The pack improves onboarding but does not create a separate rules source. |
| Called-shot AC | RAW-adapted default | Called shots use the defender's normal applicable AC. Touch called shots are checked against normal AC, and non-soft cover/concealment are adjusted where D35E exposes the needed hook. |
| Called-shot local armor piece AC | Optional advanced house rule | Disabled by default and runtime-gated by both piecemeal armor and called shots. When enabled, selected called-shot locations replace only the defender's armor/profile armor contribution with mapped local protection. The default sums applicable PAcS profile pieces and explicit armor-like values on mapped D35E native slot items; a setting can use only the highest applicable bonus globally or read per-location sum/highest overrides from the coverage editor. GMs can edit the coverage map per called-shot location and reset to the module defaults. |
| Exposed head/hand shots | Optional house rule | Disabled by default. Head/Eye/Ear can remove armor AC only when their mapped native headgear slots are empty; Hand can do the same when its mapped native handgear slots are empty. Any equipped, carried, non-melded equipment item in a mapped native slot prevents exposure and keeps the full armor bonus, whether it is native D35E, PAcS, magic gear, or custom gear. The exposed checks use the same effective native-slot coverage map that the GM can edit for local armor. |
| Helmet skill penalties | Optional house rule | Disabled by default and independent of piecemeal armor/called-shot automation. Configured PAcS Head-slot helmets can use per-item Spot/Listen values, while ordinary equipped D35E `Head` slot items can use editable default penalties; helmets do not supply local armor AC. |
...
That means some bundled piece armor values intentionally differ from Ultimate Combat. This keeps a D35E character wearing a complete catalog suit at the same armor AC as the normal 3.5e armor item, while preserving the piecemeal mechanic for swaps and mixed suits. This is still optional-rule automation: it is not exact Pathfinder RAW, and it is not official D&D 3.5 RAW.

For masterwork and magic, separately created pieces use torso > legs > arms precedence. Suit-bound magic or masterwork applies only when the whole active torso+arms+legs suit shares the same suit ID; partial suit pieces act like normal nonmagical, non-masterwork pieces. Breaking down a normal magic or masterwork full suit keeps those copied benefits suit-bound, while explicitly configured PAcS pieces can still be marked as separately enchanted. Special material automation is limited to D35E-representable effects, and material benefits require every active selected piece to share the same material.
