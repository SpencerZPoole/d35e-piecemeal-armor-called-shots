...
| Exposed head/hand shots | Optional house rule | Disabled by default. Head/Eye/Ear can remove armor AC only when D35E's native `Head` slot is empty; Hand can do the same when D35E's native `Hands` slot is empty. Any equipped native item in that slot prevents exposure. |
