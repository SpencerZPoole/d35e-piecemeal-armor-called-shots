...
| Called-shot local armor piece AC | Optional advanced house rule | Disabled by default. When enabled, selected called-shot locations replace only the defender's armor/profile armor contribution with the matching local armor piece contribution. This uses active PAcS profile data or recognized native Armor-slot baseline suits; unknown custom armor is not guessed. |
| Exposed head/hand shots | Optional house rule | Disabled by default. Head/Eye/Ear can remove armor AC only when D35E's native `Head` slot is empty; Hand can do the same when D35E's native `Hands` slot is empty. Any equipped, carried, non-melded equipment item in the exact native slot prevents exposure and keeps the full armor bonus, whether it is native D35E, PAcS, magic gear, or custom gear. |
| Helmet skill penalties | Optional house rule | Disabled by default. Configured PAcS Head-slot helmets can use per-item Spot/Listen values, while ordinary equipped D35E `Head` slot items can use editable default penalties; helmets do not supply local armor AC. |
