...
| D35E armor math | Automated through aggregate item | D35E remains responsible for final AC, ACP, max Dex, ASF, and speed calculations after the module syncs one aggregate item. |
| Local armor AC | Automated in Apply Damage | Called-shot locations can replace the aggregate armor contribution with matching piece coverage. Touch called shots are checked against normal AC. |
