...
- `D35E.DamageRoll.preHitCheck` is used for RAW-adapted called-shot defense adjustments before D35E decides hit or crit: touch called shots use normal AC, non-soft cover can be doubled, optional local armor piece AC can replace only the active armor/profile armor contribution for selected locations, and optional exposed head/hand settings can subtract only the active armor contribution when the mapped native headgear or handgear slots are empty. Any equipped, carried, non-melded equipment item in a mapped native slot prevents exposed armor loss and keeps the full armor bonus. Local armor runs first and prevents exposed head/hand from double-applying to the same hit check.
