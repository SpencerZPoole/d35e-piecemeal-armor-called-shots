...
const sourcePath = path.join(root, "data", "called-shot-feats.json");
const packPath = path.join(root, "packs", "called-shot-feats");
...
function assertProjectRoot() {
  ...
  if (relativePackPath !== "packs/called-shot-feats") throw new Error(`refusing to replace unexpected pack path: ${relativePackPath}`);
}
...
console.log(`build-called-shot-feat-pack: wrote ${documents.length} item(s) to ${path.relative(root, packPath)}`);
