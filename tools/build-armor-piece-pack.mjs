...
import { buildArmorPiecePackDocuments } from "./armor-piece-pack-documents.mjs";

const root = process.cwd();
const manifestPath = path.join(root, "module.json");
const packPath = path.join(root, "packs", "armor-pieces");
...
function assertProjectRoot() {
  ...
  if (relativePackPath !== "packs/armor-pieces") throw new Error(`refusing to replace unexpected pack path: ${relativePackPath}`);
}
...
console.log(`build-armor-piece-pack: wrote ${documents.length} item(s) to ${path.relative(root, packPath)}`);
