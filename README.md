...
Versioned release assets are published on GitHub. For v1.7.0, the release manifest is:

```text
https://github.com/SpencerZPoole/d35e-piecemeal-armor-called-shots/releases/download/v1.7.0/module.json
```
...
- Keeps called-shot hit checks on the defender's normal applicable AC by default, matching the Ultimate Combat called-shot baseline.
- Offers an advanced, disabled-by-default `Called shots use local armor piece AC` house rule with per-location toggles, editable coverage mapping, global sum/highest handling, and per-location sum/highest overrides for tables that want called shots to replace only the armor/profile armor contribution with mapped local protection.
- Lets armor components and called-shot profile locations name multiple coverage slots, such as `head; eyes; ears`.
- Offers optional, disabled-by-default exposed headshot and exposed hand-shot house rules: no mapped native slot item means no armor bonus for that called shot, while any equipped D35E, PAcS, magic, or custom item in a mapped slot keeps the full armor bonus.
- Keeps optional PAcS helmet Spot/Listen penalties for tables that want headgear to matter on skill rolls.
...
The screenshots below are cropped from a live Foundry VTT `scratch` world using D35E 3.0.2.

![Native D35E attack dialog with the PAcS Called Shot selector](docs/assets/screenshots/v1.7-called-shot-selector.png)

![D35E Apply Damage card showing called-shot AC details](docs/assets/screenshots/v1.7-apply-damage-ac-details.png)

![Actor inventory showing native Armor and PAcS armor-piece slots](docs/assets/screenshots/v1.7-actor-pacs-inventory.png)

![Local armor piece AC settings with source handling and coverage summaries](docs/assets/screenshots/v1.7-local-armor-settings.png)

![Coverage map editor with PAcS and native D35E slot choices](docs/assets/screenshots/v1.7-coverage-map-editor.png)
...
8. Use D35E's native Apply Damage button. Called shots use normal applicable AC unless an optional exposed head/hand setting or the advanced local armor piece AC setting applies.
...
Module settings live in Foundry's right sidebar gear tab under `Game Settings > D35E Piecemeal Armor And Called Shots`. The main toggles are `Enable piecemeal armor` and `Enable called shots`, so either half of the module can be used independently. Turning `Enable piecemeal armor` off is the intended called-shots-only mode: it hides `PAcS: Torso`, `PAcS: Arms`, and `PAcS: Legs` from actor sheets, suspends armor automation, and visually locks the local armor piece AC panel while preserving existing profile and local-armor preferences for later. Turning `Enable called shots` off locks called-shot-only settings such as exposed head/hand, local armor piece AC, outcome automation, full-attack behavior, feat gates, and the called-shot coverage overlay without clearing their saved values. `Called-shot full-attack feat rules` defaults to `Require feats (RAW-adapted)`, while `Warn only` and `Do not require feats` let a table loosen the full-attack permission check without granting feat bonuses. `Called-shot effect automation` defaults to `GM confirms severe effects`: normal outcomes apply after D35E Apply Damage, but critical and debilitating outcomes ask the GM before changing actor data. Optional exposed headshots, exposed hand shots, local armor piece AC, and helmet Spot/Listen penalties are separate house-rule toggles, disabled by default. Helmet Spot/Listen penalties remain editable even when piecemeal armor or called shots are disabled because they only need equipped D35E Head-slot gear. The local armor location toggles appear inline beneath `Called shots use local armor piece AC`; `Edit coverage map` opens the advanced editor for which PAcS pieces and native D35E slots can cover each location. The called-shot profile editor is available in the same settings category. Profiles are world settings, so a GM can clone or replace the bundled defaults with table-specific locations, penalties, coverage, and effects.

Called shots use the defender's normal applicable AC by default. The module still applies the RAW-adapted touch, cover, concealment, range/reach, full-attack, DR, severity, and outcome rules that D35E can represent. Coverage slot fields remain useful for profile editing, chat-card overlays, and optional house rules, but they no longer replace armor AC by body location by default. If `Called shots use local armor piece AC` is enabled, the inline child location toggles choose which locations replace the defender's armor/profile armor contribution with mapped local protection. The default handling sums all applicable local sources; `Local armor AC source handling` can switch all locations to the single highest applicable bonus, or use per-location overrides from the coverage map editor. `Edit coverage map` lets a GM adjust which PAcS pieces and native D35E slots cover each called-shot location and, when per-location handling is selected, whether each location sums sources or uses the highest source. The default map uses PAcS pieces where they physically make sense and D35E native slots where available: arms use PAcS Arms plus Wrists/Shoulders, hands use Hands/Wrists, eyes use Eyes/Head/Headband, head and ears use Head/Headband, torso locations use PAcS Torso plus Chest/Body, legs use PAcS Legs plus Feet, and neck uses Neck. This almost always lowers the target AC for called shots; it is the grittier non-RAW table option. If local armor does not apply, `Enable exposed headshots` can make Head, Eye, and Ear called shots remove the defender's active armor/profile armor contribution only when the target has no equipped item in the mapped headgear slots; mapped Eyes, Head, or Headband equipment keeps the full armor bonus. If local armor does not apply, `Enable exposed hand shots` can make Hand called shots do the same only when the target has no equipped item in the mapped handgear slots; mapped Hands or Wrists equipment keeps the full armor bonus. Shield, natural armor, Dexterity, deflection, dodge, size, and other AC sources are not removed.
