...
- Offers optional, disabled-by-default exposed headshot and exposed hand-shot house rules based on D35E's native `Head` and `Hands` slots.
...
Called shots use the defender's normal applicable AC by default. The module still applies the RAW-adapted touch, cover, concealment, range/reach, full-attack, DR, severity, and outcome rules that D35E can represent. Coverage slot fields remain useful for profile editing, chat-card overlays, and optional house rules, but they no longer replace armor AC by body location by default. If `Called shots use local armor piece AC` is enabled, the inline child location toggles choose which locations replace the defender's armor/profile armor contribution with the matching local armor piece contribution. This almost always lowers the target AC for called shots; it is an advanced non-RAW table option. If local armor does not apply, `Enable exposed headshots` can make Head, Eye, and Ear called shots remove the defender's active armor/profile armor contribution only when the target has no equipped item in D35E's native `Head` slot. If local armor does not apply, `Enable exposed hand shots` can make Hand called shots do the same only when the target has no equipped item in D35E's native `Hands` slot. Shield, natural armor, Dexterity, deflection, dodge, size, and other AC sources are not removed.
