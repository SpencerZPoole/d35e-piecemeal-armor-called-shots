...
Versioned release assets are published on GitHub. For v1.6.0, the release manifest is:

```text
https://github.com/SpencerZPoole/d35e-piecemeal-armor-called-shots/releases/download/v1.6.0/module.json
```
...
- Offers an advanced, disabled-by-default `Called shots use local armor piece AC` house rule with per-location toggles for tables that want called shots to replace only the armor/profile armor contribution with matching local piece armor.
- Lets armor components and called-shot profile locations name multiple coverage slots, such as `head; eyes; ears`.
- Offers optional, disabled-by-default exposed headshot and exposed hand-shot house rules: no equipped native slot item means no armor bonus for that called shot, while any equipped D35E, PAcS, magic, or custom item in that slot keeps the full armor bonus.
